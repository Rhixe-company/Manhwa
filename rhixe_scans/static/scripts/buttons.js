...
(function () {
  ...
  var iframeURL =
    "https://" +
    /* istanbul ignore next */ "buttons.github.io" +
    "/buttons.html";
  ...
  var useShadowDOM =
    ...
    !("prototype" in HTMLElement.prototype.attachShadow);
  ...
  var parse = function (str, sep, eq, decodeURIComponent) {
    ...
    forEach(str.split(sep), function (entry) {
      if (entry !== "") {
        ...
        obj[decodeURIComponent(ref[0])] =
          ref[1] != null
            ? decodeURIComponent(ref.slice(1).join(eq))
            : undefined;
      }
    });
    return obj;
  ...
  var onceReadyStateChange = /* istanbul ignore next: IE lt 9 */ function (
    target,
    regex,
    func
  ) {
  ...
  var getColorSchemeMediaQuery = function (
    systemColorScheme,
    widgetColorScheme
  ) {
    return (
      ...
      "){" +
      widgetColorSchemes[
        hasOwnProperty(widgetColorSchemes, widgetColorScheme)
          ? widgetColorScheme
          : systemColorScheme
      ] +
      "}"
  ...
  var getColorScheme = function (declarations) {
    ...
    return (
      widgetColorSchemes[
        hasOwnProperty(widgetColorSchemes, colorSchemes["no-preference"])
          ? colorSchemes["no-preference"]
          : "light"
      ] +
  ...
  var octicon = function (icon, height) {
    ...
    var defaultHeight =
      height >= 24 && /* istanbul ignore next */ 24 in data[icon].heights
    ...
    return (
      ...
      defaultHeight +
      '" width="' +
      (height * svg.width) / defaultHeight +
      '" height="' +
      height +
  ...
  var render$1 = function (root, options, func) {
    ...
    if (
      ("." + hostname).substring(hostname.length - domain.length) !==
      "." + domain
    ) {
    ...
    if (
      ((hostname === domain || hostname === "gist." + domain) &&
        path[3] === "archive") ||
      (hostname === domain &&
        path[3] === "releases" &&
        (path[4] === "download" ||
          (path[4] === "latest" && path[5] === "download"))) ||
    ...
    if (
      toLowerCase(options["data-show-count"]) !== "true" ||
      hostname !== domain ||
      path[1] === "marketplace" ||
      path[1] === "sponsors" ||
      path[1] === "orgs" ||
  ...
  var devicePixelRatio =
    window.devicePixelRatio || /* istanbul ignore next */ 1;

  var ceilPixel = function (px) {
    return (
      (devicePixelRatio > 1
        ? Math.ceil(
            (Math.round(px * devicePixelRatio) / devicePixelRatio) * 2
          ) / 2
        : Math.ceil(px)) || 0
    );
  };
  ...
  if (
    ...
    render$1(
      document.body,
      parse(window.name || location.hash.replace(/^#/, "")),
      function () {}
    );
  } else {
    defer(function () {
